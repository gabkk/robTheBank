...
contract RobTheBank is usingProvable{
    ...
    */
    ...
    function getBankLength()public view returns(uint256){
		return listOfBank.length;
	}

	function getListOfBank()public view returns(address[] memory){
		return listOfBank;
	}
	...
	function getBankInfos(address _bankAddr)public view returns(string memory, uint256, bool,bool){
		Bank memory p = Banks[_bankAddr];
		return (p.name, p.balance, p.isCreated, p.usingOracle);
	}

	// Get the balance of the user
	function getUserHistory(address player) public view returns(int256){
    ...
    function _oracleRandom(address _bankAddr) private{
        ...
        pendingQueries[_queryId].qPrice = provable_getPrice("Random");
        require(msg.value > pendingQueries[_queryId].qPrice, "Bet is smaller than the price for oracle");
